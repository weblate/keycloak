...
public final class KeycloakModelUtils {
     ...
     * backoff + jitter algorithm to compute the delay. More details https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/
     *
        ...
        while (true) {
            ...
            try {
                ...
                if (isExceptionRetriable(re) && ++retryCount < attemptsCount) {
                    int delay = (int)(Math.pow(2, retryCount) * retryIntervalMillis) + rand.nextInt(retryIntervalMillis);
                    try {
                        Thread.sleep(delay);
     ...
     * and must have a retriable state (for example, "40001" on CockroachDB).
     *
