...
public class HistoryPasswordPolicyProvider implements PasswordPolicyProvider {
    ...
    public PolicyError validate(RealmModel realm, UserModel user, String password) {
        ...
        if (passwordHistoryPolicyValue != -1) {
            if (user.userCredentialManager().getStoredCredentialsByTypeStream(PasswordCredentialModel.TYPE)
            ...
            if (passwordHistoryPolicyValue > 0) {
                if (this.getRecent(user.userCredentialManager().getStoredCredentialsByTypeStream(PasswordCredentialModel.PASSWORD_HISTORY),
