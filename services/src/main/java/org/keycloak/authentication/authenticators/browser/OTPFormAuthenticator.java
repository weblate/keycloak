...
public class OTPFormAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator, CredentialValidator<OTPCredentialProvider> {
    ...
    public void validateOTP(AuthenticationFlowContext context) {
        ...
        boolean valid = context.getUser().userCredentialManager().isValid(new UserCredentialModel(credentialId, getCredentialProvider(context.getSession()).getType(), otp));
        if (!valid) {
    ...
    public boolean configuredFor(KeycloakSession session, RealmModel realm, UserModel user) {
        return user.userCredentialManager().isConfiguredFor(getCredentialProvider(session).getType());
    }
