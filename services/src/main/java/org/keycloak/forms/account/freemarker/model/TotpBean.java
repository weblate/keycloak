...
public class TotpBean {
    ...
    public TotpBean(KeycloakSession session, RealmModel realm, UserModel user, UriBuilder uriBuilder) {
        ...
        this.enabled = user.userCredentialManager().isConfiguredFor(OTPCredentialModel.TYPE);
        if (enabled) {
            List<CredentialModel> otpCredentials = user.userCredentialManager().getStoredCredentialsByTypeStream(OTPCredentialModel.TYPE).collect(Collectors.toList());
