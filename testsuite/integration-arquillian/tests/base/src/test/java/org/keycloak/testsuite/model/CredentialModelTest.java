...
public class CredentialModelTest extends AbstractTestRealmKeycloakTest {
    ...
    public void testCredentialCRUD(KeycloakSession session) throws Exception {
        ...
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession currentSession) -> {
            ...
            Assert.assertFalse(user.userCredentialManager().moveStoredCredentialTo(passwordId.get(), "not-known"));

            // Assert can't move credential when not found
            Assert.assertFalse(user.userCredentialManager().moveStoredCredentialTo("not-known", otp2Id.get()));

            // Move otp2 up 1 position
            Assert.assertTrue(user.userCredentialManager().moveStoredCredentialTo(otp2Id.get(), passwordId.get()));
        });
        ...
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession currentSession) -> {
            ...
            Assert.assertTrue(user.userCredentialManager().moveStoredCredentialTo(otp2Id.get(), null));
        });
        ...
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession currentSession) -> {
            ...
            // Move password down
            Assert.assertTrue(user.userCredentialManager().moveStoredCredentialTo(passwordId.get(), otp1Id.get()));
        });
        ...
        KeycloakModelUtils.runJobInTransaction(session.getKeycloakSessionFactory(), (KeycloakSession currentSession) -> {
            ...
            // Remove password
            Assert.assertTrue(user.userCredentialManager().removeStoredCredentialById(passwordId.get()));
        });
