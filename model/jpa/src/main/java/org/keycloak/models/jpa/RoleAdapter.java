...
public class RoleAdapter implements RoleModel, JpaModel<RoleEntity> {
    ...
    public boolean isComposite() {
        Boolean composite = getEntity().isComposite();
        if (composite != null) {
            return composite;
        }
        TypedQuery<RoleEntity> query = em.createNamedQuery("getChildrenRoles", RoleEntity.class);
        query.setParameter("roleId", getId());
        query.setMaxResults(1);
        composite = !query.getResultList().isEmpty();
        getEntity().setComposite(composite);
        return composite;
    }

    @Override
    public void addCompositeRole(RoleModel role) {
        RoleEntity childEntity = toRoleEntity(role);
        em.merge(new CompositeRoleEntity(this.role, childEntity));
        getEntity().addCompositeRole(childEntity);
    }

    @Override
    public void removeCompositeRole(RoleModel role) {
        Query query = em.createNamedQuery("removeCompositeAndChildRoleEntry");
        query.setParameter("compositeId", getId());
        query.setParameter("childId", role.getId());
        query.executeUpdate();
        getEntity().removeCompositeRole(role.getId());
    }

    @Override
    public Stream<RoleModel> getCompositesStream() {
        List<RoleEntity> entities = getEntity().getComposites();
        if (entities == null) {
            TypedQuery<RoleEntity> query = em.createNamedQuery("getChildrenRoles", RoleEntity.class);
            query.setParameter("roleId", getId());
            entities = query.getResultList();
            getEntity().setComposites(entities);
        }
        return entities.stream().map(c -> new RoleAdapter(session, realm, em, c));
    }
    
    @Override
    public Stream<RoleModel> getCompositesStream(String search, Integer first, Integer max) {
        return session.roles().getRolesStream(realm,
                getCompositesStream().map(RoleModel::getId),
                search, first, max);
    }
